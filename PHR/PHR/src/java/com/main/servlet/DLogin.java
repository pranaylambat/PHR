...
public class DLogin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String dLoginName = LoginProcess.dLoginName(email_id, password);
            if (loginDoctor.trim().length() != 0) {
                request.getSession().setAttribute("docprimary", loginDoctor);
                request.getSession().setAttribute("loginName", dLoginName);
                response.sendRedirect("index.jsp");
            } else {
                response.sendRedirect("website/dlogin.jsp?flage=false");
            }
        } catch (Exception e) {
            response.sendRedirect("website/dlogin.jsp?flage=false");
